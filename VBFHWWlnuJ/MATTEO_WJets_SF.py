...
if (options.channel=="el" or options.channel=="em"):
    ...
    tmp_cut="deltaR_lak8jet>(TMath::Pi()/2.0) && TMath::Abs(deltaphi_METak8jet)>2.0 && TMath::Abs(deltaphi_Vak8jet)>2.0 && v_pt>200 && ungroomed_jet_pt>200 && l_pt>45 && pfMET>80 && jet_tau2tau1 < 0.6 && ((jet_mass_pr > 40 && jet_mass_pr < 65 )  || ( jet_mass_pr > 135 && jet_mass_pr < 150)) && (mass_lvj_type2>600 && mass_lvj_type2<2000) "; 
...
##############
    ...
    tmp_cut="deltaR_lak8jet>(TMath::Pi()/2.0) && TMath::Abs(deltaphi_METak8jet)>2.0 && TMath::Abs(deltaphi_Vak8jet)>2.0 && v_pt>200 && ungroomed_jet_pt>200 && l_pt>40 && pfMET>40 && jet_tau2tau1 < 0.6 && ((jet_mass_pr > 40 && jet_mass_pr < 65 )  || ( jet_mass_pr > 135 && jet_mass_pr < 150)) && (mass_lvj_type2>600 && mass_lvj_type2<2000) "; 
...
if __name__ == '__main__':
    ...
    OutputWJetsFileName=Control_Plots_Dir_mm+"/WJets_SideBand_output.txt";
    OutputWJetsFile=open(OutputWJetsFileName,'w+');
    ...
    for cut_counter in range(Cuts_Total_Number_mm):
        ...
        cuts_file1=Cuts_File_Dir_mm+"/MATTEO_WJets_cuts_file1_%s_%s_%s_%s_%s.txt"%(options.channel,str(Plus_Cut_Counter),DEtaCut_value,MjjCut_value,nJetsCut_value);
        output_cuts_file1=open(cuts_file1,'w+');
        output_cuts_file1.write(cut_string1);
        output_cuts_file1.close();

        cuts_file2=Cuts_File_Dir_mm+"/MATTEO_WJets_cuts_file2_%s_%s_%s_%s_%s.txt"%(options.channel,str(Plus_Cut_Counter),DEtaCut_value,MjjCut_value,nJetsCut_value);
    ...
    # Make VariableList
    FileName_VariableList_mm="cfg/DataMCComparison_InputCfgFile/MATTEO_VariableList_WJets_%s_%s_%s_%s.txt"%(options.channel,DEtaCut_value,MjjCut_value,nJetsCut_value);
    ...
    for nsample in range(Sample_Total_Number_mm):
        ...
        efficence_result_string=["RISULTATI SCALE FACTOR W+JETS (Evaluated in WJets SideBand)",
                          ...
                          " ",
                          " ",
                          "ScaleFactor WJets: %f"%Scale_W_control_factor,
                          " ",
